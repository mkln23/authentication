...
import { type CreateUserBodyType, EMAIL, type LoginBodyType } from '@/schema/reqBody.schema';
...
export class AuthService {
    static async createNewUser(userBody: CreateUserBodyType) {
        ...
        const mfaSecret = totp.generate(validatedEnv.TOTP_SECRET + ':' + ProfileAction.ACTIVATE);
        ...
        await MailService.sendMail(ProfileAction.ACTIVATE, userPayload.email, mfaSecret);
        return result;
    }
    ...
    static async verifyOTPByUserId(userId: number, action: ProfileAction, otp: string) {
        const user = await findOne(Users, {
            where: { id: userId },
        });
    ...
    private static throwErrorIfActionNotAllowed(user: Users, action: ProfileAction) {
        switch (action) {
            case ProfileAction.ACTIVATE: {
