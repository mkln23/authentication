...
import { userDoesNotExistWithGivenKey, validationError } from '@/constants/errors.constant';
...
describe('GET User by ID', () => {
    const sendRequest = async (userId: number) => {
        return await request(app).get(GET_USER_BY_ID.replace(':userId', userId.toString()));
    };
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('Successful Retrieval', () => {
        ...
        it('should return error if request without userId is requested', async () => {
            const response = await request(app).get(GET_USER_BY_ID.replace(':userId', 'hello'));

            expect(response.status).toBe(httpStatus.BAD_REQUEST);
            expect(response.body).toHaveProperty('message');
            expect(response.body.message).toBe(validationError.keyMustBeValidNumber(USERID));
        });
    });
});
