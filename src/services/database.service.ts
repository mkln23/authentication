...
export const findOne = async <Entity extends ObjectLiteral>(
    ...
    return await manager.withRepository(AppDataSource.getRepository(entity)).findOne(options);
};
...
export const find = async <Entity extends ObjectLiteral>(
    ...
    const manager = transactionManager ?? AppDataSource.manager;
    return await manager.withRepository(AppDataSource.getRepository(entity)).find(options ?? {});
};
...
export const insertOne = async <Entity extends ObjectLiteral>(
    ...
    const manager = transactionManager ?? AppDataSource.manager;
    const repository = manager.withRepository(AppDataSource.getRepository(entity));
...
export const upsertOne = async <Entity extends ObjectLiteral>(
    ...
    await repository.upsert([data] as Parameters<typeof repository.upsert>[0], conflictPaths);
