...
describe('GET all active Users', () => {
    ...
    const generateToken = async () => {
        ...
        accessToken = jwt.sign({ userId: testUser.id }, validatedEnv.JWT_ACCESS_SECRET, {
            expiresIn: '10m',
        });
    };

    const sendRequest = async () => {
        return await request(app).get(GET_ACTIVE_USERS).set('Authorization', `Bearer ${accessToken}`);
    };
